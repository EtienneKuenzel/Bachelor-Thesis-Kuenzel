...
class Networksss(nn.Module):
    ...
    def __init__(self,state_size, action_size, hidsize1=128, hidsize2=128):
        ...
        self.act_func1 = PAU(cuda=USE_CUDA).requires_grad_(True)
...
class Networkdd(nn.Module):
    #DuelDQN
    def __init__(self,state_size, action_size, freeze_pau=False, hidsize1=128, hidsize2=128):
        super(Network, self).__init__()


        self._h1val = nn.Linear(state_size, hidsize1)
        self._h2val = nn.Linear(hidsize1, hidsize2)
        self._h3val = nn.Linear(hidsize2, 1)

        self._h1adv = nn.Linear(state_size, hidsize1)
        self._h2adv = nn.Linear(hidsize1, hidsize2)
        self._h3adv = nn.Linear(hidsize2, action_size)

        nn.init.xavier_uniform_(self._h1val.weight,gain=nn.init.calculate_gain('linear'))
        nn.init.xavier_uniform_(self._h2val.weight,gain=nn.init.calculate_gain('linear'))
        nn.init.xavier_uniform_(self._h3val.weight,gain=nn.init.calculate_gain('linear'))
        nn.init.xavier_uniform_(self._h1adv.weight,gain=nn.init.calculate_gain('linear'))
        nn.init.xavier_uniform_(self._h2adv.weight,gain=nn.init.calculate_gain('linear'))
        nn.init.xavier_uniform_(self._h3adv.weight,gain=nn.init.calculate_gain('linear'))
    ...
    def forward(self, state):
        val1 = self._h1val(state)
        valh = self.act_func1(val1)
        val2 =self._h2val(valh)
        valh = self.act_func2(val2)
        val3 = self._h3val(valh)

        adv1 = self._h1adv(state)
        advh = self.act_func1(adv1)
        adv2 =self._h2val(advh)
        advh = self.act_func2(adv2)
        adv3 = self._h3val(advh)

        q = val3 + adv3 - adv3.mean()
        print(q)
        return q
